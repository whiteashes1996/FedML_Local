...
from fedml_api.standalone.fedprox.my_model_trainer_classification import MyModelTrainer as MyModelTrainerCLS
from fedml_api.standalone.fedprox.my_model_trainer_nwp import MyModelTrainer as MyModelTrainerNWP
from fedml_api.standalone.fedprox.my_model_trainer_tag_prediction import MyModelTrainer as MyModelTrainerTAG
...
def add_args(parser):
    ...
    parser.add_argument('--wd', help='weight decay parameter;', type=float, default=0.000)
...
def load_data(args, dataset_name):
    ...
    if dataset_name == "mnist":
        ...
        we uniformly sample a fraction of clients each round (as the original FedProx paper)
        """
...
def create_model(args, model_name, output_dim):
    ...
    if model_name == "lr" and args.dataset == "femnist":
        logging.info("LogisticRegression + FEMNIST")
        model = LogisticRegression(28 * 28, output_dim)
    elif model_name == "cnn_ori" and args.dataset == "mnist":
        logging.info("CNN_OriginalFedAvg + MNIST")
        model = CNN_OriginalFedAvg(output_dim)
    elif model_name == "cnn_ori" and args.dataset == "femnist":
        logging.info("CNN + FederatedEMNIST")
        model = CNN_OriginalFedAvg(output_dim)
    elif model_name == "vgg11" and args.dataset == "cifar10":
        logging.info("vgg11 + FederatedEMNIST")
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="cifar10",
        name="FedProx-mu_" + str(args.mu) + "r_"+ str(args.comm_round) + "-e_" + str(args.epochs) + "-b_" + str(args.batch_size) + "-lr_" + str(args.lr) + "-model_" + str(args.model),
        config=args
    )
    ...
    fedproxAPI = FedProxAPI(dataset, device, args, model_trainer)
    fedproxAPI.train()